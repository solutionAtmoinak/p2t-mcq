...
import { FaLock, FaLockOpen, FaQuestion, FaRegLightbulb, FaSection } from "react-icons/fa6";
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import examTimeFormatter from '../../Helper/examTimeFormatter';
import { RootState } from '../../Store/Store';
import { useFetchDbAnswersMutation, useSubmitExamMutation } from '../../Api/spAppApi';
...
const PaperPage2 = () => {
    ...
    const { selectedMcqPaper: paper, selectedInternalService, selectedPackage } = useSelector((s: RootState) => s.detailed)
    const [paperMeta, setPaperMeta] = useState<PaperMeta>({ qusCount: 0, sectionCount: 0 })
        ...
        <main className='h-screen p-6'>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
                <div
                    ...
                    <div>
                        <p className="text-secondary text-sm font-semibold mb-1">Duration</p>
                        <p className="text-3xl font-bold text-primary">{examTimeFormatter(paper.PaperDuration ?? 0)}</p>
                    </div>
                ...
                <div
                    className="p-6 rounded-2xl shadow  flex justify-between items-center">
                    <div>
                        <p className="text-secondary text-sm font-semibold mb-1">Total Questions</p>
                        <p className="text-3xl font-bold text-primary">{paperMeta.qusCount}</p>
                    </div>
                ...
                <div
                    className="p-6 rounded-2xl shadow  flex justify-between items-center">
                    <div>
                        <p className="text-secondary text-sm font-semibold mb-1">Total Marks</p>
                        <p className="text-3xl font-bold text-primary">{paper.TotalMarks}</p>
                    </div>
                    <CgSandClock className='text-primary text-6xl' />
                </div>
                <div
                    className="p-6 rounded-2xl shadow  flex justify-between items-center">
                    <div>
                        <p className="text-secondary text-sm font-semibold mb-1">Sections</p>
                        <p className="text-3xl font-bold text-primary">{paperMeta.sectionCount}</p>
                    </div>
...
interface PaperMeta { qusCount: number, sectionCount: number }
