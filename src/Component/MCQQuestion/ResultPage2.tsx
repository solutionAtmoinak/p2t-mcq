...
const ResultPage2 = () => {
  ...
  const {
    selectedOptions,
    mcqQuestionList: question,
    selectedPackageId: packageId,
  } = useSelector((state: RootState) => state.mcqQuestion);
  const { selectedMcqPaper } = useSelector(
    (state: RootState) => state.detailed,
  );
  ...
  return (
    <main className="p-4 sm:p-6">
      <header className="bg-[#DCE9F9] rounded-2xl sm:rounded-3xl p-5 sm:p-8 relative overflow-hidden shadow">
        <h1 className="font-display text-xl sm:text-2xl lg:text-3xl font-bold text-primary mb-0 break-words">
          {selectedMcqPaper.MCQPaperName}
        </h1>
      </header>

      <section className="mt-4 grid grid-cols-1 lg:grid-cols-[330px_1fr] gap-5 sm:gap-6 items-start">
        {/* gauge */}
        <div className="p-4 bg-sky-50 w-full rounded-lg flex justify-center">
          <ReactApexChart
            options={options}
            series={[percentage]}
            type="radialBar"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 w-full">
          <div className="bg-orange-100 rounded-xl px-5 sm:px-6 py-5 flex items-center justify-between w-full shadow-sm">
            <div>
              <h4 className="text-gray-500 text-sm font-bold">Answered</h4>
          ...
          <div className="bg-green-100 rounded-xl px-5 sm:px-6 py-5 flex items-center justify-between w-full shadow-sm">
            <div>
              <h4 className="text-gray-500 text-sm font-bold">Correct</h4>
              <p className="text-2xl sm:text-3xl font-semibold text-gray-800">
                {selectedOptions.filter((o) => !!o.IsCorrect).length}
              </p>
            </div>

            <div className="w-10 h-10 flex items-center justify-center rounded-full bg-green-500 text-white shrink-0">
              <IoMdDoneAll size={20} />
            </div>
          </div>

          <div className="bg-red-100 rounded-xl px-5 sm:px-6 py-5 flex items-center justify-between w-full shadow-sm sm:col-span-2 xl:col-span-1">
            <div>
              <h4 className="text-gray-500 text-sm font-bold">Un-Answered</h4>
              <p className="text-2xl sm:text-3xl font-semibold text-gray-800">
                {question.length - selectedOptions.length}
              </p>
            </div>

            <div className="w-10 h-10 flex items-center justify-center rounded-full bg-red-500 text-white shrink-0">
              <IoMdClose size={20} />
            </div>
          </div>
        </div>
